...
--------------------------------------- 2/25 (화) -----------------------------------------
	🔹 typescript 환경에서 styled-components 설치, 이해 및 활용
	...
	🔹 주요 컴포넌트 이해 및 사용하기
		- View : div 역할
		- Text : p 태그 역할
		- Pressable, TouchableOpacity : 버튼(누르면 함수 작동)
		- Image : 이미지 삽입
		- ScrollView : 긴 화면 스크롤 작동 
		- TextInput : input 태그
		- Alert : alert 역할
	...
	<TextInput>														
		onSubmitEditing													
			: 텍스트 입력중 모바일 자판의 return/done(ios) 또는  엔터키(android)를 눌렀을때												
		onChangeText													
			: 텍스트가 변할때마다 호출할 함수, 별도의 매개변수를 보내지 않아도 받는 함수에서는 매개변수로 텍스트내용이 자동으로 들어간다.												
		returnKeyType='done'													
			:모바일 자판의 엔터키 표시 내용을 변경할수 있다 (완료, 리턴 등등)												
	...
	<TextInput					
		onSubmitEditing={onSubmit} onChangeText={onChangeText} placeholder="오늘의 기분을 적어주세요.." value={feelings}				
		placeholderTextColor={colors.textGray}				
	/>					
						
	const onChangeText = (text) => setFeelings(text);					
	const onSubmit = ()=>{ '제출시 관련 코드 짜면 됨(DB 저장 등)' }					
	// textinput을 여러줄로 만들경우					
	...
	<TouchableOpacity>																						
	key																					
		: 반드시 포함되어야 하며, 고유한 id의 개념이다																				
	selected																					
		: selected={true}  또는 false 로 설정가능하며, props에서 뽑아 쓸수 있다.																				
	onPress																					
		: 눌렀을때 실행될 함수																				
	disabled={true}    한번 submit하면 true로 해주어서 2번 이상 제출 안되게 방지한다.																					
	const emotions = ["😀", "❤️", "😂", "🐱", "🐔", "🪱", "🦖"];																					
	<View>																					
	...
	<Image source={require('../assets/icons/backWhite.png')}/>				
 	<Image source={{uri : item.thumbnail}} />			
	
	
	* 1버튼 알람																
	Alert.alert( '잠깐!', '아직 좋아요 한 리포트가 없어요.',  [{text: '확인', onPress: () => {} }])																
	Alert.alert('구매 실패', '구매 중 오류가 발생하였습니다.');																
																	
	* 2버튼 알람																
	Alert.alert( //alert 사용																
		'잠깐!', '아직 좋아요 한 리포트가 없어요.', [ 															
			{text: '취소', onPress: () => {}}, 														
			{text: '확인', onPress: () => {}}, 														
		]															
	);																
